...
int main(int argc, char const *argv[]) {
  ...
  Sales_data sd8("0-211-70223-X", 4, 24.99);
  Sales_data sd9("0-211-48273-I", 4, 19.78);
  Sales_data sd10("0-211-38426-J", 7, 5.68);
  cout << hasher(sd) << endl;
  ...
  auto n = SDset.bucket_count();
  cout << n << endl;
  cout << SDset.max_bucket_count() << endl;
  for(size_t i = 0; i != n; ++i)
    cout << SDset.bucket_size(i) << endl;
  //cout << *(SDset.find(sd4)) << endl;
  cout << endl;
  for(size_t i = 0; i != n; ++i){
    cout << i << ": ";
    for(auto iter = SDset.begin(i); iter != SDset.end(i); ++iter){
      cout << *iter << ",  ";
    }
    cout << endl;
  }
